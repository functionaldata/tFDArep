...
testthat::test_that('mismatched input dimension for Ly and Lt',{
  ...
  bw_mu_min <- NULL
  testthat::expect_error(SparseProductFPCA(sSup, Lt[[1:10]], Ly, fpca.op1, fpca.op2, pc.j, pc.k, bw_mu_min, bw_mu_max))
})
...
testthat::test_that('check output eigenvalues',{
  ...
  res <- SparseProductFPCA(sGrid, Lt, Ly, fpca.op1=NULL, fpca.op2=NULL, pc.j=2, pc.k=2)

  testthat::expect_equal(c(5,2,2,1)/10, as.numeric(unlist(res$VarOrdered)/sum(unlist(res$VarOrdered))), tolerance = 0.3)
})
